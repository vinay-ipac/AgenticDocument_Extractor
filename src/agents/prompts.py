...
# Voter list specific prompt
VOTER_LIST_PROMPT = """Extract voter information from this list.

Expected fields per voter:
- Name (नाम)
- Age (आयु) / Date of Birth
- Gender (लिंग)
- Constituency (निर्वाचन क्षेत्र)
- Serial Number (क्रमांक)
- Address/Village (पता/गांव)
- EPIC No. (if visible)
...
Keep names in original script (Hindi or English)."""

# Agent details form prompt
AGENT_DETAILS_PROMPT = """Extract agent details from this form.

Expected fields:
- Agent Name (एजेंट का नाम)
- Agent ID (एजेंट आईडी)
- Contact Number (संपर्क नंबर)
- Area/Location (क्षेत्र)
- Status (स्थिति)
- Date of Registration (पंजीकरण तिथि)
